...
interface EventBase {
   ...
   * -- and a missing id is a row she cannot act on rather than a bug. Mock chat
   * does hand them out, so that acting on one is demoable without a stream. Opaque everywhere past the adapter: the shape of
